...
<div class="container">
    ...
    <?php flash('success'); ?>
    <?php flash('error'); ?>
    <form action="<?php echo BASE_URL; ?>admin/role/edit/<?php echo $data['role']['id']; ?>" method="POST">
        <div class="mb-3">
            <label for="name" class="form-label">Tên vai trò</label>
            <input type="text" name="name" id="name" class="form-control" value="<?php echo $data['role']['name']; ?>" required>
        </div>
        <div class="mb-3">
            <label for="description" class="form-label">Mô tả</label>
            <textarea name="description" id="description" class="form-control"><?php echo $data['role']['description']; ?></textarea>
        </div>
        <div class="mb-3">
            ...
            <?php foreach ($data['permissions'] as $permission): ?>
                <div class="form-check">
                    <input type="checkbox" name="permissions[]" value="<?php echo $permission['id']; ?>" id="perm_<?php echo $permission['id']; ?>" class="form-check-input" 
                        <?php echo in_array($permission['id'], $data['rolePermissions']) ? 'checked' : ''; ?>>
